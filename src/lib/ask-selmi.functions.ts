...
- Short paragraphs. Bullets when useful. Avoid corporate fluff. End with one clear, actionable next step.
...
- فقرات قصيرة، ونقاط لما تفيد. ابعد عن الكلام المؤسسي الفاضي. اختم دائمًا بخطوة عملية واحدة واضحة.
...
export const askSelmi = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const system = base + buildCourseGuidance(data.courses ?? [], data.lang);
    try {
