    ...
    <Section id="about" eyebrow={t("about_eyebrow")} title={t("about_title")}>
      <div className="mx-auto max-w-6xl">
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          {/* Left Column: Visual Typographic Anchor */}
          <div className="lg:col-span-4 relative animate-fade-in">
            <div className="absolute top-0 start-0 w-0.5 h-full bg-gradient-to-b from-[var(--gold)] via-[var(--accent)] to-transparent opacity-70" />
            <div className="ps-6 py-2">
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground block mb-1">
                {isAr ? "الخبرة والتأثير" : "Experience & Impact"}
              </span>
              <h3 className="text-4xl sm:text-5xl font-extrabold text-foreground tracking-tight font-display mb-3">
                {isAr ? "٩+ سنوات" : "9+ Years"}
              </h3>
              <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
                {isAr
                  ? "من العمل الدؤوب وتطوير رأس المال البشري في مصر، السعودية و١٢ دولة أخرى."
                  : "Of continuous leadership and talent enablement across Egypt, KSA, and 12 countries."}
              </p>
            </div>
          </div>

          {/* Right Column: The Bio Paragraph */}
          <div className="lg:col-span-8 space-y-6">
            <div className="relative">
              {/* Subtle decorative quote mark */}
              <span className="absolute -top-6 -start-4 text-6xl font-serif text-gold/10 select-none pointer-events-none">
                “
              </span>

              <p className="text-base sm:text-lg md:text-xl leading-[1.8] text-foreground/80 relative z-10 font-medium text-balance">
                {isAr ? (
                  <>
                    <span className="text-gold font-bold">رئيس قسم التعليم و التطوير</span>، وممارس في{" "}
                    <span className="text-foreground font-semibold hover:text-gold transition-colors duration-200">
                      إدارة المواهب
                    </span>{" "}
                    و{" "}
                    <span className="text-foreground font-semibold hover:text-gold transition-colors duration-200">
                    ...
                    ، بخبرة تتجاوز <span className="text-gold font-bold">٩ سنوات</span> في{" "}
                    <span className="text-foreground font-semibold">مصر</span> و{" "}
                    <span className="text-foreground font-semibold">المملكة العربية السعودية</span> و{" "}
                    <span className="text-gold font-bold">١٢ دولة أخرى</span>. خبرات تمتد عبر قطاعات{" "}
                    <span className="italic text-foreground/90 font-medium">
                      التعليم، التجزئة، السلع الاستهلاكية، والخدمات اللوجستية
                    </span>
                    ، بالشراكة مع مؤسسات مثل{" "}
                    <span className="text-foreground font-semibold hover:text-gold transition-colors duration-200">
                      KnowledgeCity
                    </span>{" "}
                    و{" "}
                  ...
                  <>
                    <span className="text-gold font-bold">Head of Education & Development</span> and a
                    hands-on practitioner in{" "}
                    <span className="text-foreground font-semibold hover:text-gold transition-colors duration-200">
                      talent and performance management
                    </span>
                    , with <span className="text-gold font-bold">9+ years</span> across{" "}
                    <span className="text-foreground font-semibold">Egypt</span>,{" "}
                    <span className="text-foreground font-semibold">Saudi Arabia</span> and{" "}
                    <span className="text-gold font-bold">12 other countries</span>. Experience spans{" "}
                    <span className="italic text-foreground/90 font-medium">
                      education, retail, FMCG and logistics
                    </span>
            ...
            {/* Bottom Row: Partners Listed as clean text links */}
            <div className="pt-4 border-t border-foreground/5 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
              <span className="font-bold uppercase tracking-wider text-gold/85">
                {isAr ? "الشركاء والجهات:" : "Partners & Clients:"}
              </span>
              <span className="hover:text-gold transition-colors duration-200 font-semibold">KnowledgeCity</span>
              <span className="text-foreground/10 select-none">/</span>
              <span className="hover:text-gold transition-colors duration-200 font-semibold">G4S</span>
              <span className="text-foreground/10 select-none">/</span>
              <span className="hover:text-gold transition-colors duration-200 font-semibold">Aramex</span>
              <span className="text-foreground/10 select-none">/</span>
              <span className="hover:text-gold transition-colors duration-200 font-semibold">Imtenan</span>
            </div>
          </div>
        </div>
      </div>
...
function CurrentCourses() {
  ...
  return (
    <Section
      ...
      title={tt("دورات مباشرة ودورات قادمة", "Live courses and upcoming courses")}
    >
      {loading ? (
          ...
          {upcoming.length > 0 && (
            <div className="mt-12">
              ...
              >
                ...
                {tt("كورسات سيتم فتح التسجيل بها قريباً", "Courses opening registration soon")}
              </h3>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {upcoming.map((c) => (
          ...
          {(course.course_goals || course.target_audience) && (
            <div className="grid sm:grid-cols-2 gap-3">
              {course.course_goals && (
                <CourseInfoBlock
                  icon={Target}
                  title={tt("مخرجات الكورس", "Course Outcomes")}
              ...
              {course.target_audience && (
                <CourseInfoBlock
                  icon={Users}
                  title={tt("هذا الكورس مناسب لـ", "This course is suitable for")}
